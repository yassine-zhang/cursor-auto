...
async function monitorBrowserPage(config: MonitorConfig) {
  try {
    ...
    async function getData(targetPage: Page) {
      try {
        const members = await targetPage.evaluate(() => {
          const memberElements = document.querySelectorAll('.flex.w-full.cursor-pointer');
          return Array.from(memberElements).map(element => {
            ...
            return {
              name: nameElement?.getAttribute('title') || nameElement?.textContent?.trim() || 'No name',
        ...
        console.log(`数据已保存: ${now.toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' })}, 共 ${members.length} 个成员`);
    ...
    if (config.continuous) {
      // 持续监控模式
      console.log(`开始持续监控，每 ${config.interval / 1000} 秒获取一次数据...`);
...
async function startMonitor() {
  try {
    ...
    let fileName = await askQuestion('请输入要保存的文件名 (默认: members-data.json): ');
    if (!fileName.trim()) {
    ...
    // 询问监控模式
    const mode = await askQuestion('选择运行模式 (1: 单次获取, 2: 持续监控) [1]: ');
